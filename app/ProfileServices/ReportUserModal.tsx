...
export default function ReportUserModal() {
  ...
  // ✅ אם נכנסים עם UID מהפרופיל – להביא אוטומטית את המשתמש
  useEffect(() => {
    console.log("uid:", uid, "username:", username);
    if (uid && username) {
        console.log("✅ ReportScreen opened with UID:", uid);
      setSelectedUser({ id: uid, username, profileImage: '' });
    }
  }, [uid, username]);

  useEffect(() => {
    if (!uid && searchQuery.length > 2) {
  ...
  const renderUserItem = ({ item }: { item: User }) => (
    <TouchableOpacity
      style={[
        styles.userItem,
        { borderColor: theme.colors.border, backgroundColor: selectedUser?.id === item.id ? theme.colors.primaryLight : 'transparent' }
      ]}
...
const styles = StyleSheet.create({
  container: { flex: 1, justifyContent: 'flex-start', alignItems: 'center', paddingTop: 50, direction: 'rtl' },
  header: { width: '100%', flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingVertical: 12, borderBottomWidth: 1 },
  headerTitle: { fontSize: 18, fontWeight: '600', flex: 1, textAlign: 'center' },
  closeButton: { padding: 8 },
  content: { width: '100%', padding: 20 },
  label: { fontSize: 16, fontWeight: 'bold', marginBottom: 10, textAlign: 'right' },
  searchInput: { height: 40, borderWidth: 1, borderRadius: 8, paddingHorizontal: 15, marginBottom: 10, textAlign: 'right' },
  loadingIndicator: { marginVertical: 10 },
  userList: { maxHeight: 200, marginBottom: 10 },
  userItem: { paddingVertical: 10, paddingHorizontal: 15, borderWidth: 1, borderRadius: 8, marginBottom: 5, flexDirection: 'row', alignItems: 'center', justifyContent: 'flex-end' },
  userInfo: { flexDirection: 'row-reverse', alignItems: 'center' },
  userText: { fontSize: 16, marginRight: 10 },
  selectedUserContainer: { padding: 10, borderRadius: 8, borderWidth: 1, marginBottom: 10, alignItems: 'center' },
  selectedUserText: { fontSize: 16, fontWeight: 'bold' },
  reasonsContainer: { flexDirection: 'row-reverse', flexWrap: 'wrap', justifyContent: 'flex-end', marginBottom: 20 },
  reasonButton: { paddingVertical: 8, paddingHorizontal: 15, borderRadius: 20, borderWidth: 1, marginHorizontal: 4, marginVertical: 4 },
  reasonText: { fontSize: 14 },
  otherInput: { height: 100, borderWidth: 1, borderRadius: 8, padding: 10, textAlignVertical: 'top', textAlign: 'right' },
  sendButton: { width: '90%', padding: 15, borderRadius: 10, alignItems: 'center', position: 'absolute', bottom: 20 },
  sendButtonText: { color: '#fff', fontSize: 18, fontWeight: 'bold' },
  emptyListText: { textAlign: 'center', marginTop: 20 },
});
