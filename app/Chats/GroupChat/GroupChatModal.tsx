...
import {
  ...
  Modal, // Use React Native's built-in Modal
  Platform,
  StatusBar,
  StyleSheet,
...
// import Modal from 'react-native-modal'; // REMOVED
...
const GroupChatModal = () => {
    ...
    const getGroupImageUrl = async (groupId: string) => {
        ...
        try {
            const groupDocRef = doc(db, 'group_chats', groupId);
            const docSnap = await getDoc(groupDocRef);
            if (docSnap.exists()) {
                const data = docSnap.data();
                return data.groupImage || null;
            }
            return null;
        } catch (e) {
            console.warn(`Error fetching group image from Firestore for ${groupId}:`, e);
            return null;
        }
    };
    ...
    return (
        ...
        >
            ...
            <Modal
                animationType="slide"
                transparent={false}
                visible={isGroupImageModalVisible}
                onRequestClose={closeGroupImageModal}
            >
                <GroupImageModal
                    groupImageUrl={groupImageUrl}
                    isUploading={isUploading}
